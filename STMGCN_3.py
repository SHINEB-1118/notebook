...
# COG/Heading은 원형 평균 방식 반영

RESAMPLE_INTERVAL = "5min"

def circular_mean_deg(series):
    radians = np.radians(series.dropna().values)
...
def resample_ais_5min(df):
    ...
    for mmsi, group in df.groupby("MMSI"):
        ...
        lat_lon_sog = group[["lat", "lon", "sog"]].resample(RESAMPLE_INTERVAL).mean()

        cog = group["cog"].resample(RESAMPLE_INTERVAL).apply(circular_mean_deg)
        heading = group["heading"].resample(RESAMPLE_INTERVAL).apply(circular_mean_deg)

        resampled = lat_lon_sog.copy()
        resampled["cog"] = cog
...
# 11. Next-step sequence + graph pre-compute
# 과거 10개 시점 → 다음 5분 위치 예측

def create_next_step_sequences_with_graph(raw_df, norm_df, seq_len=10):
    sequences = []
...
train_sequences = create_next_step_sequences_with_graph(train_raw, train_norm, seq_len=SEQ_LEN)
val_sequences = create_next_step_sequences_with_graph(val_raw, val_norm, seq_len=SEQ_LEN)
test_sequences = create_next_step_sequences_with_graph(test_raw, test_norm, seq_len=SEQ_LEN)
